...
- **実装待ち**：パラメータ調整、表情・動作の差分、簡単なレイヤー分割、口腔パーツの分離、髪の分離と遮蔽補完、手を振る動作、尻尾の変形と物理、生成画像と九軸調整。
- **現状では不可**：パーツの頂点単位の精密変形（Warp / Mesh の各点を個別に操作し、変形を正しく制御する必要があります）。

> [!WARNING]
> 実装待ちの項目はエンドツーエンドで極めて不安定です。プログラムのデバッグが目的でない限り、試すことはお勧めしません。MCP インターフェースが存在しても、Agent がタスクを完了できるとは限りません。生成・配置・修正の繰り返しによって大量のトークンや画像生成枠を短時間で消費し、利用できる結果が得られない可能性があります。
...
- **作例と知見の共有**：PSD2Live で制作したモデル、つまずいた点、有効だったレイヤー分けのコツ。

PSD2Live のバージョン、OS、PSD のレイヤー構造と命名、再現手順、期待した結果と実際の結果、ログドックの出力やスクリーンショットがあると、原因の特定が大幅に早くなります。情報が揃っていなくても大丈夫です。一緒に確認しながら進めます。
