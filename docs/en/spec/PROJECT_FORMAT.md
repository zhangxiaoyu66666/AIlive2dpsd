...
> This branch retains its existing tools alongside the upstream authoring tools (`inspect`, `deform`, `form`, `rig`, `view`, `parameter`, `asset`, `physics`, `appearance`, `revision`, `path`). All project tools use explicit tab addressing in the desktop app. See [Project tabs](../../project-tabs.md) for routing and compatibility, and the [upstream MCP interface contract](../../zh/agent/MCP_AUTHORING.md) for authoring request shapes.
