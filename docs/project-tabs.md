...
- 文件菜单“全部保存”（Ctrl+Alt+S）依次保存所有已修改或尚未保存的工程，跳过空白和已保存且无修改的标签；已有路径直接保存，首次保存使用原有位置选择框。全部成功后返回原标签，取消或失败则停止并保留所有标签。Ctrl+S 和 Ctrl+Shift+S 仍分别保存当前工程、另存为。
- 点击窗口关闭按钮或文件菜单“退出”，有未保存修改时统一选择“全部保存并关闭 / 全部不保存 / 取消”。全部保存成功且没有新增修改才关闭；选择位置时取消、保存失败、保存期间又有修改或任何标签仍有操作进行中，均保留所有工程。
- 菜单打开工程、导入 PSD 使用新标签；已经打开的路径会切回原标签。拖入一个文件则在当前标签打开，保留标签位置和 ID，并直接显示预览。
- 启动和新建空白标签默认进入“预览”；See-Through API 页面通过工作区标签手动切换。PSD 直接分析并进入“预览”，在 See-Through 页面确认并导入后也进入预览。
