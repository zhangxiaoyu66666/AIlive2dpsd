...
- 标签超出窗口宽度时，普通鼠标滚轮、横向滚轮和下方滚动条均可横向滚动；新建、选中或返回工程时，当前标签自动滚入可见区域。滚动位置不随工程内容重建而重置。
- “新建标签 +”或 Ctrl+N 创建空工程；Ctrl+W 关闭当前标签。
- 菜单打开工程、导入 PSD 使用新标签；已经打开的路径会切回原标签。拖入一个文件则在当前标签打开，保留标签位置和 ID，并直接显示预览。
- PSD 直接分析并进入“预览”，不再转到 See-Through API 候选页。空白新标签仍提供 See-Through 入口；在该页确认并导入后也进入预览。
- 文件拖放绑定 Compose 内容区。当前工程有未保存修改时先提示保存、放弃或取消；候选文件加载成功后才替换，失败保留原工程。加载期间显示“正在打开”，拒绝关闭或重复打开；若原工程在加载期间又被修改则拒绝替换。成功替换后旧 MCP 凭证失效，需要重新认领；其他标签的工程不受影响。一次拖入多个文件会提示逐个打开。
...
工程文件及导出目录在工程存活期间保留路径占用，包括该工程之前的另存为位置。拒绝其他标签写入同一个规范化路径或重叠的导出目录。关闭工程或在当前标签成功打开另一工程后释放旧路径。同一个源 PSD 若需分别编辑，应先复制为不同的工程文件，并选择不同导出目录。
